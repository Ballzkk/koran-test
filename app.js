...
const app = {
    ...
    renderAvatar(elementId, user) {
        ...
        if (avatarUrl && avatarUrl.trim() !== '') {
            el.innerHTML = `<img src="${avatarUrl}" alt="Avatar" class="w-full h-full object-cover rounded-full">`;
        } else {
            el.innerText = letter;
        }
    },
    ...
    async fetchUserProfile(userId, email) {
        ...
        let bestStandardScore = 0;
        let bestStandardAccuracy = 0;
        let bestStandardConsistency = 100;
        ...
        try {
            ...
            if (profile) {
                ...
                bestStandardScore = profile.best_standard_score || 0;
                bestStandardAccuracy = profile.best_standard_accuracy || 0;
                bestStandardConsistency = profile.best_standard_consistency || 100;
    ...
    async processSessionUser(authUser, isFreshLogin = false) {
        ...
        if (profile) {
            // CASE 1: Profile exists with this email
            ...
            app.state.user = {
                ...
                usernameLastChanged: profile.username_changed_at || null,
                best_standard_score: profile.best_standard_score || 0,
                best_standard_accuracy: profile.best_standard_accuracy || 0,
                best_standard_consistency: profile.best_standard_consistency || 100,
            ...
            // CASE 2: Profile does NOT exist with this email
    ...
    navigate(viewId) {
        ...
        setTimeout(async () => {
            ...
            if (viewId === 'login' && this.state.skipResetLoginOnce) {
                this.state.skipResetLoginOnce = false;
    ...
    async renderDashboardHero() {
        ...
        if (user.isGuest) {
            if (greetingEl) greetingEl.innerHTML = `Welcome, <span class="text-cyan-400">${displayName}</span> 👋`;
        ...
        if (greetingEl) greetingEl.innerHTML = `Welcome back, <span class="text-cyan-400">${displayName}</span> 👋`;
    ...
    async renderHistoryStatistics() {
        ...
        this.state.history.forEach((h) => {
            ...
            row.innerHTML = `
                <td class="p-3 font-medium text-slate-100">${h.date || '—'}</td>
                <td class="p-3"><span class="bg-cyan-500/10 text-cyan-400 text-[10px] px-2 py-0.5 rounded font-bold">${h.mode || 'Standard'}</span></td>
                ...
                <td class="p-3 text-center text-slate-400">${h.consistency || 100}%</td>
            `;
        ...
        if (recentContainer) {
            ...
            if (recent.length === 0) {
                ...
                recent.forEach(h => {
                    ...
                    const mode = h.mode || 'Standard Test';
                    const scoreVal = h.score !== undefined ? h.score : (h.total_answered || h.totalAnswered || 0);
                    const accVal = h.accuracy || 0;
                    const dateStr = h.created_at ? new Date(h.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : (h.date || '—');

                    tr.innerHTML = `
                        <td class="p-3 font-semibold text-slate-200">${mode}</td>
    ...
    achievements: {
        ...
        getEvaluatedListForUser(targetUser, tests = []) {
            ...
            const bestScore = historyScores.length > 0 ? Math.max(...historyScores) : 0;

            const historyAcc = tests.map(h => parseFloat(h.accuracy) || 0);
            const bestAcc = historyAcc.length > 0 ? Math.max(...historyAcc) : 0;

            const historyCons = tests.map(h => parseFloat(h.consistency) || 0);
            const bestCons = historyCons.length > 0 ? Math.max(...historyCons) : 0;

            const streak = targetUser?.current_streak || (totalTests > 0 ? 1 : 0);
        ...
        getEvaluatedList() {
            ...
            const totalTests = history.length > 0 ? history.length : (user?.total_tests || 0);
            ...
            const bestScore = historyScores.length > 0 ? Math.max(...historyScores, user?.best_standard_score || 0, user?.high_score || 0) : (user?.best_standard_score || user?.high_score || 0);

            const rawRank = user?.rank ? String(user.rank).replace('#', '') : '9999';
            const userRank = parseInt(rawRank) || 9999;

            const historyAcc = history.map(h => parseFloat(h.accuracy) || 0);
            const bestAcc = historyAcc.length > 0 ? Math.max(...historyAcc, parseFloat(user?.best_standard_accuracy) || 0, parseFloat(user?.accuracy) || 0) : (parseFloat(user?.best_standard_accuracy) || parseFloat(user?.accuracy) || 0);

            const historyCons = history.map(h => parseFloat(h.consistency) || 0);
            const bestCons = historyCons.length > 0 ? Math.max(...historyCons, parseFloat(user?.best_standard_consistency) || 0, parseFloat(user?.consistency) || 0) : (parseFloat(user?.best_standard_consistency) || parseFloat(user?.consistency) || 0);

            const streak = user?.current_streak || (totalTests > 0 ? 1 : 0);
    ...
    try {
        ...
        if (type === 'register') {
            ...
            // Simpan profil ke tabel profiles (username dari input, display_name = null)
            if (data.user) {
                try {
                    console.log('[DEBUG Register] Insert profile:', {
                        id: data.user.id,
                        username: username,
                        display_name: null,
                    ...
                    if (profileError) {
                        console.warn('[DEBUG Register] Client-side profile insert ditolak (biasanya RLS/unauthenticated):', profileError);
                    } else {
                        console.log('[DEBUG Register] Profile berhasil dibuat saat pendaftaran.');
                    }
                } catch (err) {
            ...
            // Autofill email di form login untuk kemudahan user
            const loginEmailInput = document.getElementById('login-email');
            if (loginEmailInput) loginEmailInput.value = email;

            app.state.skipResetLoginOnce = true;
            app.navigate('login');
            return;
        }
        ...
        async saveProfile() {
            ...
            const newBio = rawBio && rawBio.trim() !== '' ? rawBio.trim() : null;
    ...
    leaderboard: {
                    ...
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="4" class="p-10 text-center space-y-3">
                                ...
                                <button onclick="app.navigate('test')" class="px-5 py-2.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-black text-xs rounded-xl transition shadow-lg shadow-cyan-500/20 inline-flex items-center gap-2 mt-2">
            ...
            if (top100.length === 0) {
                ...
                tableItems.forEach((entry) => {
                    ...
                    const avatarHtml = entry.avatar_url
                        ? `<img src="${entry.avatar_url}" alt="Avatar" class="w-8 h-8 rounded-full object-cover">`
                        : `<div class="w-8 h-8 bg-cyan-500/20 text-cyan-400 font-bold rounded-full flex items-center justify-center text-xs border border-cyan-500/30 flex-shrink-0">${effectiveName.charAt(0).toUpperCase()}</div>`;
                        ...
                        <td class="p-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[11px] text-slate-500 font-mono">@${entry.username || 'user'}</span>
                                </div>
                            </div>
                        </td>
        ...
        async openProfilePreview(userIdOrSpecial) {
            ...
            el('profile-preview-consistency', `${consistency}%`);
